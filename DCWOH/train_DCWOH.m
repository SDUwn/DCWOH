...
%     alpha2 = param.alpha2;
    ...
    %% iterative optimization
        for iter = 1:param.iter
            ...
            % update F
                F1=((XTrain_t*G+DD{1,1}))*pinv(G'*G+DD{1,5});
                F2=((YTrain_t*G+DD{2,1}))*pinv(G'*G+DD{1,5});
            ...
            for i = 1:n
                ...
                tmp=eye(c);
                for j=1:c
                    obj(j,1) = obj(j,1) + alpha1*norm(XTrain_t(:,i)-F1(:,j))^2 + alpha2*norm(YTrain_t(:,i)-F2(:,j))^2;
                end
                [~, min_idx] = min(obj);
                G(i,:)=tmp(min_idx,:);
            end
            ...
            %update V
            Z=kesi*nbits*(B*ZTrain_t'+DD{1,7})*ZTrain_t+sigma*B+beta*P*G';
            ...
            % update B
            Q = kesi*nbits*(V*ZTrain_t'+DD{1,6})*ZTrain_t+sigma*V+lamda*W'*XTrain_t;
            B=sign(Q);
            
            %update P
            P=(V*G+DD{1,8})/(G'*G+DD{1,5}+thetaP*eye(c));
            
            %%update W
            W=(XTrain_t*XTrain_t'+DD{1,3}+thetaW*eye(dX))\(XTrain_t*B'+DD{1,2});
        ...
        DD{1,1} = XTrain_t*G+DD{1,1};
        DD{2,1} = YTrain_t*G+DD{2,1};
        DD{1,2} = XTrain_t*B'+DD{1,2};
        DD{2,2} = YTrain_t*B'+DD{2,2};
        DD{1,3} = XTrain_t*XTrain_t'+DD{1,3};
        DD{2,3} = YTrain_t*YTrain_t'+DD{2,3};
        DD{1,4}= B*G+DD{1,4};
        DD{1,5} = G'*G+DD{1,5};
        DD{1,6} = V*ZTrain_t'+DD{1,6};
        DD{1,7} = B*ZTrain_t'+DD{1,7};
        DD{1,8} = V*G+DD{1,8};
        DD{1,9} = B*G+DD{1,9};
        %%
        BB{1,end+1}=B;
